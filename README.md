...
- 保持同一 pod 内排班整齐——只忌讳【可以避免却把 1 张台孤零零留在另一个班次】，其余怎么拆都行
  （大小不同的 pod 标准不同，见下）；拆成的两个班次如果【首尾正好衔接】（交班顺、无缝隙），
...
## Pod 拆分：只罚"可以避免的落单" + 衔接顺不顺

一个 pod 分成 2 个班次时，两件事各算各的罚分：

**1. 有没有台被落单，而且这本来能避免。** 只罚"少数侧只有 1 张台"，且仅当这个
pod 大小下本来可以有更不落单的拆法（`min(c, pod_size-c) >= 2`）时才罚
（`model.pod_penalty_schedule`）；只要没有台落单，不管比例多悬殊都不罚：

| pod 大小 | 会被罚的拆法 | 不罚的拆法 |
|---|---|---|
| 1 / 2 / 3 | 无（这几个大小下落单没法避免，见下） | 1+1、1+2 等唯一可能的拆法 |
| 4 | 1+3 | 2+2 |
| 5 | 1+4 | 2+3 |
| 6 | 1+5 | 2+4、3+3 |

`PENALTY_UNEVEN_SPLIT`（默认 30）是唯一的罚分数值——不罚就是 0，没有中间档。
`pod_size <= 3` 时任何拆分都不罚：1、2 张台没有"另一半"可言，3 张台唯一能做
的拆分（1+2）本身就带着"少数侧=1"，但没有更公平的替代方案，落单在那个大小下
是没法避免的，不该罚；4 张台起，"少数侧=1"就是可以避免的（比如 6 张台完全可以
拆成 2+4 或 3+3，1+5 是自己选的），才罚。
...
pod 的拆分有没有命中）。这条只在第 1 点算出了非零罚分时才有意义——本来就不罚的
拆法（比如任何大小下"没人落单"的拆法）打折也还是 0。
...
| `PENALTY_UNEVEN_SPLIT` | 30 | pod 拆分里有台落单、且本可避免（其余拆法一律不罚） |
...
```bash
uv sync --extra dev
uv run pytest                # 41 个测试；test_model.py 需要 ortools，缺则自动跳过
